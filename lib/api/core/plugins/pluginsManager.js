...
function PluginsManager (kuzzle) {
  ...
  this.run = function pluginRun () {
    ...
    pluginsWorker = _.pickBy(this.plugins, plugin => plugin.threads > 0);
    ...
    return new Promise((resolve, reject) => {
      async.forEachOf(this.plugins, this.asyncRunPlugin.bind(this), (err) => {
        if (err) {
          return reject(err);
        }
        resolve({});
  ...
  this.asyncRunPlugin = function pluginAsyncRunPlugin (plugin, callback) {
    let
      pipeWarnTime = this.config.common.pipeWarnTime,
      pipeTimeout = this.config.common.pipeTimeout;

    if (plugin.threads) {
      debug('[%s] starting worker with %d treads', plugin.name, plugin.threads);

      initWorkers(plugin, this.kuzzle.config)
        .then(() => callback())
        .catch(err => {
          this.silent || console.log('Plugin', plugin.name, ' errored with message: ', err.message, '. Skipping...'); // eslint-disable-line no-console
          callback();
        });
    ...
    try {
      ...
      console.warn(`WARNING: Unable to init plugin ${plugin.name}: ${e.message}`, e.stack); // eslint-disable-line no-console
      callback();
      return true;
    }
    ...
    if (plugin.object.controllers) {
      initControllers(this.controllers, this.routes, plugin, plugin.name);
    }
   ...
   * @param {object} config
   */
  this.registerPlugin = function pluginRegisterPlugin (name, constructor, config = {}) {
    this.plugins[name] = {name, object: new constructor(), config, threads: 0};
  };

  /**
   * @param {string} name
   * @param {object} pluginPath
   * @param {object} config
   * @param threadCount
   */
  this.registerThreadPlugin = function pluginRegisterThreadPlugin (name, pluginPath, config = {}, threadCount = 1) {
    const realPath = path.resolve(this.kuzzle.rootPath, pluginPath);
    let object;

    try {
      object = loadPluginFromPath(realPath, config);
    } catch (e) {
      console.error(`Unable to load plugin ${name}. ${e}`); // eslint-disable-line no-console
      return;
    }

    this.plugins[name] = {name, path: pluginPath, object, config, threads: threadCount};
  };
}
...
function initWorkers (plugin, kuzzleConfig) {
  return pm2Promise
    ...
    .catch(err => {
      if (err) {
        return Promise.reject(new PluginImplementationError('Error while starting worker plugin: '.concat(plugin.name)));
      }
    });
}
 ...
 * @param {object[]} threadPlugins
 */
function loadThreadPlugins(rootPath, threadPlugins) {
  let loadedPlugins = {};

  debug('loading thread plugins:\n%O', threadPlugins.map(p => p.name));

  threadPlugins.forEach(pluginRegistration => {
    const pluginPath = path.resolve(rootPath, pluginRegistration.path);
    let plugin = null;

    try {
      plugin = loadPluginFromPath(pluginPath, pluginRegistration.config);
    } catch (e) {
      console.error(`Unable to load plugin ${pluginRegistration.name}. ${e}`); // eslint-disable-line no-console
      return;
    }

    loadedPlugins[plugin.name] = plugin;
    loadedPlugins[plugin.name].threaded = true;
  });
 ...
 * @returns {{name: string, object: object, config: object, path: string}}
 */
function loadPluginFromPath(pluginPath, pluginConfig) {
